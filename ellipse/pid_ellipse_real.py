...
class ControlTurtlebot():
    def __init__(self, initial_position, theta, final_position, kp, ki, kd, alpha, i, write_flag, previous_time, dt=0.1):
	...
	start = time.time()
	    ...
	    self.ts.append(time.time()-start+float(previous_time))
	...
	if self.write_flag:
		mode = 'w'
		if os.path.exists('{}.txt'.format(self.filename)):
			mode = 'a'

		with open('{}.txt'.format(self.filename), mode) as file:
...
def generate_elliptic_waypoints(a, b, n, filename):
    waypoints = [[1, 0.1, 0.05, 0.5, a, 0, filename]]
    for i in range(n):
        theta = 2 * math.pi * i / n
        x = a * math.cos(theta)
        y = b * math.sin(theta)
	print(x, y)
        waypoints.append([1, 0.1, 0.05, 0.5, x, y, filename])
    waypoints.append([1, 0.1, 0.05, 0.5, a, 0, filename])
    return waypoints

if __name__ == "__main__":
    a = 1.0  # Semi-major axis
    b = 0.5  # Semi-minor axis
    n = 13   # Number of waypoints
    filename = 'pid_fractional_ellipse_real'
    temp = open('{}.txt'.format(filename), 'w')
    temp.close()

    data = generate_elliptic_waypoints(a, b, n, filename)

    try:
	write_flag = False
	previous_time = 0.0
        for kp, ki, kd, alpha, final_x, final_y, filename in data:
            bot = ControlTurtlebot([0, 0], 0, [final_x, final_y], kp=kp, kd=kd, ki=ki, alpha=alpha, i=filename, write_flag=write_flag, previous_time=previous_time)
	    spanned = bot.run(previous_time)
	    print(spanned)
	    previous_time = spanned
	    write_flag = True
            rospy.sleep(1)
