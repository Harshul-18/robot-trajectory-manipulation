...
class VirtualRobotController(RobotController):
    """
    Controller for the virtual Turtlebot3 robot in Gazebo
    """
    def __init__(self, initial_position, theta, final_position, pid_controller, filename, dt=0.1, enable_dynamic_plot=True, expected_trajectory=None):
        """
        Initialize the virtual robot controller
        ...
        super(VirtualRobotController, self).__init__(
        ...
        self.set_robot_position(initial_position[0], initial_position[1], theta)
    ...
    def set_robot_position(self, x, y, theta):
        """
        Set the robot position in Gazebo
        ...
        try:
            ...
            rospy.loginfo("Robot position set to ({}, {}, {})".format(x, y, theta))
...
def run_line_trajectory(controller_type, final_position, num_points=16, output_dir=None, kp=0.5, ki=0.1, kd=0.2, alpha=0.5, enable_dynamic_plot=True):
    ...
    if output_dir is None:
        output_dir = os.path.join('..', 'results', 'virtual', 'line')
...
def run_square_trajectory(controller_type, side_length, num_points=16, output_dir=None, kp=0.5, ki=0.1, kd=0.2, alpha=0.5, enable_dynamic_plot=True):
    ...
    # Create output directory if it doesn't exist
    if output_dir is None:
        output_dir = os.path.join('..', 'results', 'virtual', 'square')
    os.makedirs(output_dir, exist_ok=True)
...
def run_ellipse_trajectory(controller_type, a, b, num_points=16, output_dir=None, kp=0.5, ki=0.1, kd=0.2, alpha=0.5, enable_dynamic_plot=True):
    ...
    # Create output directory if it doesn't exist
    if output_dir is None:
        output_dir = os.path.join('..', 'results', 'virtual', 'ellipse')
    os.makedirs(output_dir, exist_ok=True)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run virtual robot experiments')
