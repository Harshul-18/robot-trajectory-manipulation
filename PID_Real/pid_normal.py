...
class ControlTurtlebot():
	...
	self.ts = []
        ...
        self.pid_distance = PID(Kp=self.kp, Ki=self.ki, Kd=self.kd, setpoint=0)
        self.pid_heading = PID(Kp=self.kp, Ki=self.ki, Kd=self.kd, setpoint=0)
...
if __name__ == "__main__":
    data = [
        # kp, ki, kd, final_x, final_y, filename
        [1, 0.1, 0.05, 0.5, 0.75, 'pid_normal'],
    ]

    try:
        for kp, ki, kd, final_x, final_y, filename in data:
            ControlTurtlebot([0, 0], 0, [final_x, final_y], kp=kp, kd=kd, ki=ki, i=filename)
            rospy.sleep(1)
