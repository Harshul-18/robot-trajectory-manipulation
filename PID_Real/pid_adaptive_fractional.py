...
class AdaptiveFractionalPID:
    ...
    def __call__(self, current_value):
        ...
        # Adjust alpha based on error dynamics (simple example)
        self.alpha = self.adjust_fractional_order(self.alpha, error)
    ...
    def adjust_fractional_order(self, order, error):
        # Simple example: decrease order if error is large, increase if small
        if abs(error) > 0.1:
            order = max(0.1, order - 0.01)
        else:
            order = min(1.0, order + 0.01)
        return order
...
class ControlTurtlebot():
	...
	self.ts = []
        ...
        self.pid_distance = AdaptiveFractionalPID(Kp=self.kp, Ki=self.ki, Kd=self.kd, alpha=self.alpha, setpoint=0)
        self.pid_heading = AdaptiveFractionalPID(Kp=self.kp, Ki=self.ki, Kd=self.kd, alpha=self.alpha, setpoint=0)
...
if __name__ == "__main__":
    data = [
        ...
        [1, 0.1, 0.05, 0.5, 0.5, 0.75, 'pid_adaptive_fractional'],
    ]
