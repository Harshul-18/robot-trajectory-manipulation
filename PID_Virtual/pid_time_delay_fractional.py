...
class TimeDelayFractionalPID:
    def __init__(self, Kp, Ki, Kd, alpha, tau, setpoint=0, output_limits=(None, None)):
        self.Kp = Kp
        self.Ki = Ki
        self.Kd = Kd
        self.alpha = alpha
    ...
    def __call__(self, current_value):
        ...
        output = self.Kp * self.predict_error(error) + self.Ki * integral + self.Kd * derivative
    ...
    def predict_error(self, error):
        # Simple prediction based on the delay tau
        return error * (1 + self.tau)

class ControlTurtlebot():
    def __init__(self, initial_position, theta, final_position, kp, ki, kd, alpha, tau, i, dt=0.1):
	...
	self.ts = []
        ...
        self.pid_distance = TimeDelayFractionalPID(Kp=self.kp, Ki=self.ki, Kd=self.kd, alpha=self.alpha, tau=self.tau, setpoint=0)
        self.pid_heading = TimeDelayFractionalPID(Kp=self.kp, Ki=self.ki, Kd=self.kd, alpha=self.alpha, tau=self.tau, setpoint=0)
...
if __name__ == "__main__":
    data = [
        # kp, ki, kd, alpha, tau, final_x, final_y, filename
        [1, 0.1, 0.05, 0.5, 0.1, 0.5, 0.75, 'pid_time_delay_fractional'],
    ]

    try:
        for kp, ki, kd, alpha, tau, final_x, final_y, filename in data:
            ...
            ControlTurtlebot([0, 0], 0, [final_x, final_y], kp=kp, kd=kd, ki=ki, alpha=alpha, tau=tau, i=filename)
            rospy.sleep(1)
